...
// an exception that is thrown when a player has to perform a coup but tries to perform another action instead
class must_perform_coup_exception : public std::invalid_argument{
    public:
        explicit must_perform_coup_exception(const std::string &playerName, int numCoins) : 
        std::invalid_argument(playerName + ", you have " + std::to_string(numCoins) + " coins, you must attempt to perform a coup") 
        {
...
// an exception that is thrown when a player tries to target a null player
class null_player_exception : public std::invalid_argument{
    public:
        explicit null_player_exception() : 
        std::invalid_argument("The player cannot be null") 
        {
...
// an exception that is thrown when a player tries to perform an action but the game is already over
class game_over_exception : public std::runtime_error{
    public:
        explicit game_over_exception() : 
        std::runtime_error("The game is already over") 
        {
            
        }
};

// an exception that is thrown when there is an invalid attempt to copy the game
class invalid_game_copy_exception : public std::runtime_error{
    public:
        explicit invalid_game_copy_exception(const std::string &reason) : 
        std::runtime_error("Cannot copy the game because the game " + reason) 
        {
