...
namespace coup{
    ...
    int Game::playAction(const string &action, Player *target, bool undone){
        // if the game is already over, throw an exception
        if(this->isGameOver())
        ...
        // check that the action is valid based on the game rules
        if(!this->isValidAction(action))
            throw must_perform_coup_exception(currentPlayer->getName(), currentPlayer->coins());
