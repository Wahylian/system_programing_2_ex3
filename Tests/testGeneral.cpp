...
TEST_CASE("Test General Special Interactions"){
    SUBCASE("Test Undo Coup"){
        ...
        SUBCASE("Test Undo Coup Performed by Another Player"){
            SUBCASE("Test With Enough Coins"){
                SUBCASE("Test Undo Coup Performed on Self"){
                    ...
                    // give g1 enough coins to undo the coup
                    g1.prepareForTurn();
                    g1.tax();
                    g1.prepareForTurn();
                    g1.tax();
                    ...
                    // check that g1 has 5 coins
                    CHECK(g1.coins() == 5);
                    // check that g1 has no actions (as undo does not require an action)
                    CHECK(g1.getRemainingActions() == 0);

                    // make g1 undo the coup performed by g2 on g1
                    CHECK_NOTHROW(g1.undoCoup(g2)); // should not throw an exception

                    // check that g1 has no coins left
                    CHECK(g1.coins() == 0);
                }
                SUBCASE("Test Undo Coup Performed on Another Player"){
                    ...
                    // check that g1 has 0 coins left
                    CHECK(g1.coins() == 0);
                }
            }
            SUBCASE("Test Without Enough Coins"){
                SUBCASE("Test Undo Coup Performed on Self"){
                    General g1 = General("John");
                    General g2 = General("Doe");
                    ...
                    // make g1 undo the coup performed by g2 on g1
                    CHECK_THROWS(g1.undoCoup(g2)); // should throw an exception as g1 doesn't have 5 coins
